...
import { generateSeamlessSpeechMismatch } from "../csv/generateSeamlessSpeechMismatch"
...
export default function Page() {
  ...
  const handleValidate = async (e) => {
    ...
    try {
      ...
      for (let i = 0; i < csvList.length; i++) {
        ...
        setCsvList((prev) => prev.map((item, idx) => (idx === i ? { ...item, state: "loading" } : item)))
        ...
        setCsvList((prev) =>
          prev.map((item, idx) =>
            idx === i ? { ...item, state: res.success ? "success" : "error", errorMsg: res.success ? "" : res.msg } : item
          )
        )
      ...
      if (isAllValid) setValidState({ type: "success", msg: "Validate success, continue with generate studies" })
    } catch (error) {
  ...
  const handleUpload = async (e) => {
    ...
    try {
      ...
      const pageList = generateSeamlessSpeechMismatch(studiesCSV, videoOrigin, videoMismatch, studiesID, studyConfig, attentionCheckList, includeAttentionChecks)
  ...
  return (
    <>
      ...
      <h2 className="font-semibold tracking-tight text-slate-900 dark:text-slate-100 mt-10 border-b pb-1 text-3xl border-neutral-200/70">
        Upload Seamless Speech Mismatch CSV Studies
      </h2>
      <p className="mt-3 text-sm text-gray-500">
        Study type is fixed to <code>{STUDY_KEY}</code>. CSV columns: <code>[clip_origin, system, clip_mismatch]</code>. Origin videos are pulled from <code>{VIDEO_TYPE_ORIGIN}</code>; mismatch videos from <code>{VIDEO_TYPE_MISMATCH}</code>.
      </p>
          ...
          {loadedCSV && (
            <div className="flex flex-col gap-4">
              ...
              <div className="flex flex-col gap-8 mt-4 items-center">
                {isValid ? (
                  <button
                    className="cursor-pointer flex h-10 items-center gap-2 w-44 justify-center rounded-md border border-transparent bg-primary-600 px-4 py-2 text-lg font-bold text-white sm:text-sm transition-all"
                  ...
                  <button
                    className="flex cursor-pointer h-10 items-center gap-2 w-44 justify-center rounded-md border border-transparent bg-green-600 px-4 py-2 text-base font-bold text-white sm:text-sm transition-all"
